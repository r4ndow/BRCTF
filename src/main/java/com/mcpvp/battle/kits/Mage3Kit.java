...
public class Mage3Kit extends BattleKit {
    ...
    class DamageSpell extends CooldownItem {
        ...
        public void onUse(PlayerInteractEvent event) {
            ...
            arrow.setShooter(Mage3Kit.this.getPlayer());
            Mage3Kit.this.attach(arrow);
            Vector v = arrow.getVelocity().clone().add(arrow.getVelocity().clone());

            Mage3Kit.this.attach(EasyTask.of(task -> {
            ...
            Mage3Kit.this.attach(new InteractiveProjectile(this.getPlugin(), arrow)
                    .onDeath(() -> {
                        FireworkEffect effect = FireworkEffect.builder()
                                .with(FireworkEffect.Type.BALL)
                                .withColor(
                                        Mage3Kit.this.getGame().getTeamManager().getTeam(Mage3Kit.this.getPlayer()).getColor().getColor(),
                                        Color.PURPLE
                                )
                                .build();
                    ...
                    .onDamageEvent(ev -> {
                        ...
                        if (Mage3Kit.this.isTeammate(hit)) {
                            return;
                        }
    ...
    class FlameSpell extends CooldownItem {
        ...
        public void onUse(PlayerInteractEvent event) {
            ...
            pearl.setShooter(Mage3Kit.this.getPlayer());
    ...
    class LightningSpell extends CooldownItem {
        ...
        public void onUse(PlayerInteractEvent event) {
            if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                ...
                Mage3Kit.this.getGame().getWorld().playSound(
                        Mage3Kit.this.getPlayer().getEyeLocation(),
                        Sound.SHOOT_ARROW,
                        1f,
                        0.5f
                );
                Egg egg = event.getPlayer().launchProjectile(Egg.class);
                egg.setShooter(Mage3Kit.this.getPlayer());
                egg.setVelocity(egg.getVelocity().multiply(1));
                Mage3Kit.this.attach(egg);

                Location spawned = event.getPlayer().getLocation().add(0, 1, 0);

                // Optional lifetime clamp was here using MAX_SPELL_PROJECTILE_DIST_SQUARED.

                Mage3Kit.this.attach(new InteractiveProjectile(this.getPlugin(), egg)
        ...
        private void strike(Location location) {
            ...
            LightningStrike lightning =
                    Mage3Kit.this.getPlayer().getWorld().strikeLightningEffect(block.getLocation());
            ...
            for (Player enemy : Mage3Kit.this.getEnemies()) {
                ...
                if (Mage3Kit.this.getGame().getTeamManager()
                ...
                new Mage3Kit.Mage3StrikeEvent(enemy).call();
            }
        }
    ...
    class FreezeSpell extends CooldownItem {
        ...
        public void onUse(PlayerInteractEvent event) {
            ...
            snowball.setShooter(Mage3Kit.this.getPlayer());
            snowball.setVelocity(snowball.getVelocity().multiply(1));
            Mage3Kit.this.attach(snowball);
            ...
            Mage3Kit.this.attach(new InteractiveProjectile(this.getPlugin(), snowball)
                    .onDeath(() -> {
        ...
        private void freezePlayer(Player player) {
            ...
            Mage3Kit.IceBoxStructure iceBox = new Mage3Kit.IceBoxStructure(player);
            Mage3Kit.this.placeStructure(iceBox, player.getLocation().getBlock());
        }
    ...
    class IceBoxStructure extends Structure {
        ...
        public IceBoxStructure(LivingEntity target) {
            super(Mage3Kit.this.getBattle().getStructureManager(), Mage3Kit.this.getPlayer());
            this.target = target;
        ...
        public Plugin getPlugin() {
            return Mage3Kit.this.getPlugin();
        }
    ...
    class HealSpell extends CooldownItem {
        ...
        @Override
        public void onUse(PlayerInteractEvent event) {
            PotionEffect effect = new PotionEffect(PotionEffectType.REGENERATION, Duration.seconds(6).toTicks(), 3);

            if (EventUtil.isRightClick(event)) {
                ThrownPotion potion = event.getPlayer().launchProjectile(ThrownPotion.class);
                Mage3Kit.this.attach(potion);
                ...
                Mage3Kit.this.getPlayer().getWorld().playEffect(Mage3Kit.this.getPlayer().getLocation(), Effect.POTION_BREAK, PotionEffectType.REGENERATION.getId());
                EntityUtil.getNearbyEntities(Mage3Kit.this.getPlayer().getLocation(), Player.class, 2, 1, 2).stream()
                        .filter(Mage3Kit.this::isTeammate)
                        .map(Mage3Kit.this.getBattle().getKitManager()::get)
    ...
    public static class Mage3StrikeEvent extends EasyEvent {
